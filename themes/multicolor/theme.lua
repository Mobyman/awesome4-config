...
-- Textclock
os.setlocale(os.getenv("LC_TIME")) -- to localize the clock
...
theme.weather = lain.widget.weather({
    city_id = 498817, -- Saint-P
    lang = 'ru',
    notification_preset = { font = "GohuFont 10", fg = theme.fg_normal },
...
local bat = lain.widget.bat({
    settings = function()
        ...
        local function hide()
            if widget.notification ~= nil then
        ...
        local function show()
            ...
            widget.notification = naughty.notify({
                text = text .. "Осталось: " .. bat_now.time,
                timeout = 1,
                screen = mouse.screen,
        ...
        widget:set_markup(markup.fontfg(theme.font, color, perc))
    end
})

local net_widgets = require("net_widgets")
local net = net_widgets.wireless({
    font = theme.font,
    interface = "wlp2s0",
    indent = 0,
})
...
--    s.quake = lain.util.quake({ app = awful.util.terminal })
    ...
    s.mylayoutbox:buttons(my_table.join(awful.button({}, 1, function() awful.layout.inc(1) end),
